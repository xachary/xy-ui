<template>
  <div class="lazy-load" :style="{ width: width, height: height }">
    <img v-lazy="src" :class="{ 'lazy-cover': size === 'cover', 'lazy-contain': size === 'contain' }" :error-img="errorImg" :data-id="id" ref="img" />
    <xy-spinner :show="true" style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%);" color="#666"></xy-spinner>
  </div>
</template>
  ...
  export default {
    ...
    mounted() {
      ...
      for (let p in this.data) {
        this.$refs.img.dataset[p] = this.data[p]
      }
      // { bindType, el, naturalHeight, naturalWidth, $parent, src, loading, error }
      this.$Lazyload.$on('loaded', ({ el, naturalHeight, naturalWidth, src }, formCache) => {
        if (!formCache) {
          that.$emit('on-load', { el, naturalHeight, naturalWidth, data: el.dataset, src })
        }
      })
    },
    methods: {},
